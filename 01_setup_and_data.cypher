...
MATCH (p1:Player {username: 'Kuba_Kiler'}), (t1:Team {name: 'Real Madrid'}) CREATE (p1)-[:FOLLOWS]->(t1);
MATCH (p2:Player {username: 'Analityk99'}), (t5:Team {name: 'Manchester City'}) CREATE (p2)-[:FOLLOWS]->(t5);
MATCH (p3:Player {username: 'LuckyShot'}), (t1:Team {name: 'Real Madrid'}) CREATE (p3)-[:FOLLOWS]->(t1);